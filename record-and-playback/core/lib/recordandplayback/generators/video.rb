...
module BigBlueButton
  ...
  def BigBlueButton.process_webcam_videos(target_dir, temp_dir, meeting_id, output_width, output_height, audio_offset, processed_audio_file, video_formats=['webm'], split_users=[])
    ...
    # Add a nil element to the array for single-video-file users to collect all
    # remaining videos together
    split_users << nil
    ...
    split_users.each do |single_user|
      ...
      formats.each do |format|
        filename = BigBlueButton::EDL::encode(
          processed_audio_file, user_video_file, format, target_filename, audio_offset)
      end
    end
  end
