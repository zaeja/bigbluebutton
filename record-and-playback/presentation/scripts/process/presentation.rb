  ...
  begin
    ...
    if !Dir["#{raw_archive_dir}/video/*"].empty? or
      ...
      if metadata_split_webcam_user.nil?
        BigBlueButton.logger.info("No users found for individual webcam videos.")
        split_users = []
      else
        split_users = [metadata_split_webcam_user.value]
        BigBlueButton.logger.info("Will split webcam video for user #{split_users}")
      end
      ...
      BigBlueButton.process_webcam_videos(target_dir, temp_dir, meeting_id, webcam_width, webcam_height, presentation_props['audio_offset'],
        processed_audio_file, presentation_props['video_formats'], split_users)
    end
